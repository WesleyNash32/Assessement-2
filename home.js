...
//CODE HERE
const greetUser = (username) => `Welecome back, ${username}`

// console.log(greetUser('Wesley'))
...
// CODE HERE
function canWeDeliver(zipCode) {
    for(let i = 0; i < deliveryAreaZipCodes.includes; i++){
            if (zipCode !== deliveryAreaZipCodes.length){
                return `Sorry, we can't deliver to that address`
            }
        }
...
//CODE HERE
let disTitle = deals.title;
const changeTitle = title.replace('15', '10')
...
//CODE HERE
let secDeal = deals.desc;
const monthChange = desc.replace('March', 'April')